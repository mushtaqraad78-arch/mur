...
import { RADAR_VIOLATION_NAMES } from '../constants';
import type { Page, RadarViolationData } from '../types';
import { PageHeader, ActionButton } from './SharedComponents';

interface RadarsSummaryViewProps {
    navigate: (page: Page) => void;
    data: { name: string; violations: RadarViolationData[] }[];
}
...
export const RadarsSummaryView: React.FC<RadarsSummaryViewProps> = ({ navigate, data }) => {
    return (
        <div>
            <PageHeader title="مجموع الرادارات الكلي" onBack={() => navigate('home')} />
            <div className="bg-white p-4 rounded-lg shadow-lg printable-content">
                 <div className="flex flex-wrap gap-2 mb-6 justify-center no-print">
                    ...
                    <ActionButton 
                        ...
                        onClick={() => exportTableToExcel('totalRadarsTable', 'total_radars_report')}
                        colorClass="bg-green-600 text-white hover:bg-green-700"
                    />
                </div>
                <TotalRadarsTable data={data} />
            </div>
        </div>
    );
};

const TotalRadarsTable = ({ data }: { data: { name: string; violations: RadarViolationData[] }[] }) => {
    const aggregatedData = useMemo(() => {
        const totals = new Map<string, { morningCount: number; eveningCount: number; morningAmount: number; eveningAmount: number }>();
        
        RADAR_VIOLATION_NAMES.forEach(name => {
    ...
    return (
        <div>
            <h3 className="text-2xl font-bold text-center mb-4 text-[#0A1A3D]">جدول مجموع مخالفات الرادارات</h3>
            <div className="overflow-x-auto">
                <table id="totalRadarsTable" className="min-w-full bg-white border border-gray-300 text-center">
